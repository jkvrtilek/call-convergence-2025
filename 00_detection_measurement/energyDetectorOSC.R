# G. Smith-Vidaurre and J. Vrtilek
# R translation of Eric Fossler-Lussier's original Python energy detector with modifications
# 07 September 2020


energyDetector <- function(wavs, path, outpath1, outpath2, lowerfreq, higherfreq, energyoffset, amplitudethreshold, spec_fft_size, spec_fft_shift, spec_energy_floor, truncate, shouldplot, strategy, boundarystrategy, date){
  ...
  WavError <- file.path(path, paste("energyDetectorLog_", date, ".txt", sep = ""))
  ...
  invisible(pblapply(1:length(wavs), function(w){
    ...
    if(!is(obj,"try-error")) {
      ...
      write.csv(detections, file = file.path(outpath1, paste("automatic_detections_", wavs[w], ".csv", sep = "")), row.names = FALSE)
      write.csv(detections, file = file.path(outpath2, paste("automatic_detections_", wavs[w], ".csv", sep = "")), row.names = FALSE)
    } else {
