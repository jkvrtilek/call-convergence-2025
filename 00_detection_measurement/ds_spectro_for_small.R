# load packages
library(tidyverse)
library(warbleR)
library(soundgen)

# load arguments from sbatch
args = commandArgs(trailingOnly=TRUE)

# read in data
extSelTable <- readRDS(paste("/fs/ess/scratch/PAS1986/downExtSelTables/small/", args[1], sep = ""))

# remove clipped selections
extSelTable <- extSelTable %>% 
  filter(clipped == "N")

# standardize sampling rate
# extSelTable <- resample_est_waves(extSelTable, samp.rate = 250, avoid.clip = TRUE)
# test for clipping post-resampling?

# create wav.size variable for filtering
attrcheck <- attr(extSelTable, "check.results") %>% 
  select(sound.files, wav.size)

# filter out WAVs that are too small/short
extSelTable <- extSelTable %>% 
  left_join(attrcheck) %>% 
  filter(wav.size > 0.000001) %>% 
  filter(selection_length > 0.0005)

# Arguments
# X = one extended selection table; looping is in sbatch command
# bp = 5 to 100 kHz
# wl = 512 (default)
# threshold = 10, from Gerry's 2012 PLOS One paper
# fast = FALSE
# path = directory containing all extended selection tables
# ovlp = 50 (default), from Gerry's 2012 PLOS One paper
# wn = "blackman", from Gerry's 2012 PLOS One paper
specan <- spectro_analysis(extSelTable, path = "/fs/ess/scratch/PAS1986/extSelTables",
                           bp = c(5,100),
                           wl = 512,
                           threshold = 10,
                           fast = FALSE,
                           ovlp = 50,
                           wn = "blackman")

saveRDS(specan, file = paste("/fs/ess/scratch/PAS1986/downSpecan/small/spectro_analysis_", args[1], sep = ""))